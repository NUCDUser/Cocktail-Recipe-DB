...
const CocktailListBase = ({ queryBaseURL, query }) => {
  ...
  async function getCocktailsList() {
    const response = await fetch(queryBaseURL + query);
  ...
  return (
    <Container>
      ...
      {cocktailList.length == 0 ? (
        ...
        <p>No cocktails with that query were found</p>
      ) : (
...
export default CocktailListBase;
